    ...
    paymentsDB.all(sql, [], async (err, rows) => {
        ...
        for (let index = 0; index < rows.length; index++) {
            ...
            cupsDB.all(cups_sql, [element.transaction_id], (err, cups_rows) => {
                ...
                if (total_pris  === total_rendu){
                    tout_rendu = true;
                    status_payment = 'rendu';
                }else{
                    tout_rendu = false;
                    status_payment = 'partiellement rendu';
                }
                ...
                try {
                    ...
                    const update_sql = `UPDATE payments SET amount_captured = ? , status =? WHERE transaction_id = ?`;
                    paymentsDB.run(update_sql, [amount_to_capture / 100, status_payment, element.transaction_id], function(err) {
                        if (err) {
                            console.error(err.message);
                            return;
                        }
