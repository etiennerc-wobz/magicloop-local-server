...
app.post('/start-location', (req, res) => { 
    const data = req.body;
    const status = 'en cours';
    const select_sql = `SELECT cup_id FROM transactions WHERE cup_id = ?`;
    const insert_sql = `INSERT INTO transactions(transaction_id, cup_id, status) VALUES(?, ?, ?)`;
...
/*
// Handle end-location
app.post('/end-location', (req, res) => {
    console.log("///End location");
    const cup_ids = req.body.cup_list;
    const status = 'rendu';
    const select_sql = `SELECT status FROM transactions WHERE cup_id = ?`;
    ...
    cup_ids.forEach(cup_id => {
        cupsDB.get(select_sql, [cup_id], (err, row) => {
            ...
            if (row && row.status === status) {
                ...
                const update_sql = `UPDATE transactions SET status = ? WHERE cup_id = ?`;
